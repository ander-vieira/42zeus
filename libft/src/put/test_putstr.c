...
/*   Updated: 2024/01/26 11:59:53 by andeviei         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	test_putstr_fd_child1(void)
{
	...
	close(fd[1]);
	tlib_print_test(read(fd[0], buf, 10) == 5);
	...
	ft_putstr_fd("", fd[1]);
	close(fd[1]);
	tlib_print_test(read(fd[0], buf, 10) == 0);
	tlib_print_test(tlib_alloc_count() == 0);
	close(fd[0]);
	return (0);
}
...
{
	tlib_print_test(tlib_run_process(&test_putstr_fd_child1) == 0);
	tlib_print_test(tlib_run_process(&test_putstr_fd_child2) == 0);
}

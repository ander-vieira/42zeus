...
static void	test_strlcpy_testone(char *dst, char *src, size_t n, char *expected_str) {
	...
	tlib_testmalloc_leak("ft_strlcpy(%p, %S, %z)", dst, src, n);
}
...
void	test_strlcpy(void) {
	...
	tlib_testprocess_segfault(&test_strlcpy_child2, "ft_strlcpy(NULL, \"ABC\", 5)");
	tlib_testprocess_segfault(&test_strlcpy_child3, "ft_strlcpy(\"ABC\", NULL, 5)");
	tlib_testprocess_segfault(&test_strlcpy_child4, "ft_strlcpy(NULL, NULL, 5)");
}
